...
const { slug, loadProfile } = useProfile()
const config = useRuntimeConfig()

const canvasRef = ref<HTMLCanvasElement | null>(null)
const profileUrl = computed(() => `${config.public.appUrl}/${slug.value}`)

async function renderQR() {
  if (!canvasRef.value || !slug.value) return
...
function downloadPNG() {
  ...
  a.download = `nicetomeetyou-${slug.value}.png`
...
<template>
  <div class="flex h-[calc(100vh-56px)] items-center justify-center">
    <div class="flex flex-col items-center gap-6 rounded-2xl border border-warm-border bg-warm-card p-10 shadow-sm">
      ...
      <p v-if="slug" class="text-sm text-warm-muted">{{ profileUrl }}</p>
      <button
